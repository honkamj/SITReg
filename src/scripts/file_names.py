...
def optimizer_state_file_name(epoch: int, dump: bool = False) -> str:
...
def model_state_file_name(epoch: int, dump: bool = False) -> str:
...
def find_largest_epoch(training_directory: str, require_optimizer: bool = True) -> Optional[int]:
    ...
    for entry in listdir(training_directory):
        if isfile(join(training_directory, entry)):
            ...
            if (
                epoch is not None
                and (largest_epoch is None or epoch > largest_epoch)
                and (
                    not require_optimizer
                    or isfile(join(training_directory, optimizer_state_file_name(epoch)))
                )
            ):
                largest_epoch = epoch

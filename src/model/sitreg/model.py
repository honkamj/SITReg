...
class SITReg(Module):
    ...
    Arguments:
        ...
        n_transformation_convolutions_per_resolution: Defines how many convolutions to
            ...
            resolution. If None is given for some resolution, no deformation is
            extracted for that.
        max_displacements: Defines the max displacement in voxel coordinates for
            each resolution for the individual deformation predictions. If None
            is given for some resolution, no deformation is extracted for that.
    ...
    def __init__(
        ...
        max_displacements: Sequence[Optional[float]],
        ...
        if (
            n_blocks != len(max_displacements)
            or n_blocks != len(n_transformation_features_per_resolution)
            or n_blocks != len(n_transformation_convolutions_per_resolution)
        ):
            raise ValueError(
        ...
        for (n_transformation_features, max_displacement, predict_dense_for_stage,) in zip(
            ...
            if (
                ...
                and (max_displacement is None or n_transformation_features is None)
            ) or (
                not predict_dense_for_stage
                and (max_displacement is not None or n_transformation_features is not None)
            ):
                raise ValueError(
                    "Either all max displacement, number of tranformation convolutions, and number "
                    "of transformation features must be provided for a resolution or none of them."
                )
        ...
        self._dense_extraction_networks = [
            _DenseExtractionNetwork(
                ...
                max_displacement=max_displacement,  # type: ignore
        ...
        self._output_downsampling_factors = [
            ...
            for (predict_deformation, downsampling_factor,) in zip(
                predict_dense_for_stages,
                feature_downsampling_factors,
            )
            if predict_deformation
...
class _DenseExtractionNetwork(_BaseTransformationExtractionNetwork):
    ...
    def _obtain_displacement_field(self, combined_input: Tensor) -> Tensor:
        ...
        output = self._max_displacement * tanh(output)
        ...
        upsampled_output = self._upsampler(output)[self._cropping_tuple] * upsampling_factor_tensor
